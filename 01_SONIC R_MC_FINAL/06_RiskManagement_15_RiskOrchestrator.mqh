...
public:
    ...
    SAccountRiskStatus GetRiskStatus() const { return m_status; }
    SRiskParameters GetRiskParameters() const { return m_params; }
    ...
    void UpdatePeriodLosses()
    {
        ...
        // Check for new day (safe by struct)
        MqlDateTime dtCur, dtDaily; TimeToStruct(currentTime, dtCur); TimeToStruct(m_dailyStartTime, dtDaily);
        if(dtCur.day != dtDaily.day) {
            m_dailyStartBalance = m_accountBalance;
            m_dailyStartTime = currentTime;
        }

        // Check for new week (safe by struct)
        MqlDateTime dtWCur, dtWPrev; TimeToStruct(currentTime, dtWCur); TimeToStruct(m_weeklyStartTime, dtWPrev);
        if(dtWCur.day_of_week < dtWPrev.day_of_week) {
            m_weeklyStartBalance = m_accountBalance;
            m_weeklyStartTime = currentTime;
        }

        // Check for new month (safe by struct)
        MqlDateTime dtMCur, dtMPrev; TimeToStruct(currentTime, dtMCur); TimeToStruct(m_monthlyStartTime, dtMPrev);
        if(dtMCur.mon != dtMPrev.mon) {
