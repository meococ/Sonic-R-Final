...
//| Copyright 2024, Đại Bàng Dev                                    |
//+------------------------------------------------------------------+
#property copyright "Copyright 2024, Đại Bàng Dev"
...
::Print("[� � MONTE CARLO] Failed to update historical returns");
return false;
}
...
::Print(StringFormat("[🎯 MONTE CARLO] Simulation complete | VaR: %.2f%% | ES: %.2f%% | MaxDD: %.2f%%",
...
PrintFormat("[� ️ CORRELATION] Failed to get returns for %s", m_symbols[s]);
return false;
}
...
PrintFormat("[� ️ CORRELATION] %s highly correlated with %s (%.2f)",
...
::PrintFormat("[🛡� EXPOSURE BLOCK] Maximum exposure limit reached: %s%%", DoubleToString(m_monteCarloAnalysis.GetCurrentExposure() * 100, 2));
...
if(!m_monteCarloAnalysis.CheckExposureLimit()) {
::PrintFormat("[🛡� RISK BLOCK] Real-time exposure limit exceeded: %.2f%%", m_monteCarloAnalysis.GetCurrentExposure() * 100);
return true;
}

// 1. Check Monte Carlo risk metrics (Enhanced with 20k simulations)
if(m_monteCarloAnalysis.GetVaR() > 0.15) { // 15% VaR limit
::PrintFormat("[🛡️ RISK BLOCK] High VaR detected: %.1f%%", m_monteCarloAnalysis.GetVaR() * 100);
return true;
}

// 2. Check maximum drawdown
if(m_monteCarloAnalysis.GetMaxDrawdown() > 0.25) { // 25% max drawdown
::PrintFormat("[🛡️ RISK BLOCK] High projected drawdown: %.1f%%", m_monteCarloAnalysis.GetMaxDrawdown() * 100);
return true;
}
...
::PrintFormat("[🛡� RISK BLOCK] Low Kelly confidence: %.1f%%", kellyConfidence * 100);
return true;
}
...
::PrintFormat("[🛡� RISK BLOCK] High portfolio risk: %.1f%%", portfolioRisk * 100);
return true;
}
...
::PrintFormat("[🛡� RISK BLOCK] Signal confidence too low: %.1f%%", signalConfidence * 100);
return true;
}
...
::PrintFormat("[🚀 DYNAMIC STOP] ATR: %.5f | Multiplier: %.2f | Distance: %.5f | Stop: %.5f",
...
if(slope > 0.01) return 1.2; // Tang r?i ro n?u du?ng cong l�n
...
{
    // 1. T�nh to�n d? cong equity curve
    ...
    if(convexity > 0.05) {
        ...
        return 0.8;  // �u?ng cong nh? ti�u c?c
    }
    else {
        Print("? [PHASE 3] Strong negative curve - reducing risk multiplier to 0.5");
        return 0.5;  // �u?ng cong m?nh ti�u c?c - gi?m r?i ro
    }
}
...
double CalculatePositionSize()
{
    ...
    // 4. T�nh to�n position size cu?i c�ng
