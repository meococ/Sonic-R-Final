...
//+------------------------------------------------------------------+
//| ?? CONFLICT TYPES - PH�N LO?I XUNG �?T                          |
//+------------------------------------------------------------------+
...
struct SConflictData
{
   ...
   // Conflict details
   string conflictDescription;                // M� t? xung d?t
   double conflictSeverity;                   // M?c d? nghi�m tr?ng (0-1)
   int conflictingComponents;                 // S? component xung d?t
   
   // Resolution details
   ENUM_SIGNAL_TYPE resolvedSignal;           // Signal sau khi gi?i quy?t
   double resolutionConfidence;               // �? tin c?y gi?i quy?t
   string resolutionReason;                   // L� do gi?i quy?t
...
private:
   // Configuration
   double m_conflictThreshold;                // Ngu?ng ph�t hi?n xung d?t
   double m_severityThreshold;                // Ngu?ng nghi�m tr?ng
   double m_abstainThreshold;                 // Ngu?ng t? ch?i giao d?ch
   
   // Historical performance tracking
   double m_componentReliability[5];          // �? tin c?y t?ng component
   int m_componentSuccessCount[5];            // S? l?n th�nh c�ng
   ...
   int m_resolvedConflicts;                   // S? xung d?t d� gi?i quy?t
...
public:
   ...
   //+------------------------------------------------------------------+
   //| ?? DETECT CONFLICT - PH�T HI?N XUNG �?T                       |
   //+------------------------------------------------------------------+
   bool DetectConflict(SConflictData& conflict)
   {
   ...
   //+------------------------------------------------------------------+
   //| ?? ANALYZE CONFLICT SEVERITY - PH�N T�CH M?C �? NGHI�M TR?NG  |
   //+------------------------------------------------------------------+
   void AnalyzeConflictSeverity(SConflictData& conflict)
   {
      double severity = 0.0;
   ...
   //+------------------------------------------------------------------+
   //| ?? APPLY RESOLUTION STRATEGY - �P D?NG CHI?N LU?C GI?I QUY?T   |
   //+------------------------------------------------------------------+
   void ApplyResolutionStrategy(SConflictData& conflict, ENUM_MARKET_REGIME marketRegime)
   {
   ...
   //+------------------------------------------------------------------+
   //| ?? RESOLVE BY RELIABILITY - GI?I QUY?T THEO �? TIN C?Y         |
   //+------------------------------------------------------------------+
